...
if TYPE_CHECKING:
    from src.python.dto.AnimeTagData import AnimeTagData
...
class AnimeTagModel(BaseModel, DatabaseCreator.Model):
    ...
    @staticmethod
    def convert_to_dto(model: "AnimeTagModel") -> "AnimeTagData":
        from src.python.dto.AnimeTagData import AnimeTagData
        converted_dict = BaseModel.delete_model_dict_key(
            model_object_dict = model.get_all_data_by_dict()
        )

        return AnimeTagData.create_object(**converted_dict)
